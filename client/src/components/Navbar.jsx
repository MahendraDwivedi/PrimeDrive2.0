...
//       e.preventDefault();
//       setShowPopup(true);
//       setTimeout(() => setShowPopup(false), 3000); // auto-hide popup
//     } 
// }
...
//           exit={{ opacity: 0 }}
//           className="absolute top-0 right-0 mt-10 mr-4 bg-red-500 text-white px-4 py-2 rounded shadow-md z-50"
//         >
...
const Navbar = () => {
  ...
  return (
    ...
    >
      <Link to="/">
        <motion.h1 whileHover={{ scale: 1.05 }} className="font-semibold text-3xl bg-gradient-to-r from-indigo-500 via-purple-800 to-pink-700 bg-clip-text text-transparent italic">
          PRIMEDRIVE
        </motion.h1>
      </Link>
      ...
      <div className={`max-sm:fixed max-sm:h-screen max-sm:w-full max-sm:top-16 max-sm:border-t border-borderColor right-0 flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-8 max-sm:p-4 transition-all duration-300 z-50 ${location.pathname === "/" ? "" : "bg-white"} ${open ? "max-sm:translate-x-0" : "max-sm:translate-x-full"}`}>
        ...
        <div className='flex max-sm:flex-col items-start sm:items-center gap-6'>
          ...
          {location.pathname === "/" && (
            <button className='hidden md:block' onClick={() => setIsDark(!isDark)}>
              {!isDark ? <Moon className="text-yellow-300" /> : <Sun className="text-orange-500" />}
            </button>
          )}
        </div>
      </div>
      ...
      <div className='flex'>
        {location.pathname === "/" && (
          <button onClick={() => setIsDark(!isDark)}>
            {!isDark ? <Moon className="text-yellow-300 sm:hidden cursor-pointer m-2" /> : <Sun className="text-orange-300 m-2 sm:hidden cursor-pointer" />}
          </button>
        )}
        <button className='sm:hidden cursor-pointer' aria-label="Menu" onClick={() => setOpen(!open)}>
